...
module.exports.run = async (bot, message, args) => {
    ...
    var balance = await userBank.findOne({ gid: message.guild.id, user_id: message.author.id });
        ...
        let amount = args[0];
            ...
            if(!amount) return message.channel.send(`:x: **Veuillez indiquer la somme que vous souhaitez déposer !**`)
            ...
            if(isNaN(amount)) {
                ...
                banqueAmount = parseInt(banque) + parseInt(liquide)
                liquideAmount = parseInt(liquide) - parseInt(liquide)
        ...
        .setDescription(`:white_check_mark: Dépôt effectué avec succès ${message.author}.`)
        .setThumbnail('https://zupimages.net/up/20/28/f8f7.png')
        .setTimestamp()
